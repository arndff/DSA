...
public:
	void intersection(std::ofstream&, size_t) const;
	void print() const;
...
template <class KeyType, class ValueType>
void HashTable<KeyType, ValueType>::intersection(std::ofstream& ofs, size_t freq) const
{
	for (size_t i = 0; i < m_table.size(); ++i)
	{
		for (const KeyValue& pair : m_table[i])
		{
			if (pair.m_value == freq)
			{
				ofs.write((const char*)& pair.m_key, sizeof(KeyType));
			}
		}
	}
}
